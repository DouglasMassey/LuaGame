...
function love.draw()
    -- Draw ripples first (behind other content)
    ...
    if isMouseMoving then
        ...
        if rippleSpawnTimer >= rippleSpawnInterval then
            ...
            if (math.random() < 0.2) then
                local newRipple = {
                    x = mouseX + (math.random() * 20 - 10), -- Random offset between -10 and 10
                    y = mouseY + (math.random() * 20 - 10), -- Random offset between -10 and 10
                    currentFrame = 0,
